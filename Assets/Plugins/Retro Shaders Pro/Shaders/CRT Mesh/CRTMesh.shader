Shader "Retro Shaders Pro/CRT (Mesh)"
{
    ...
    SubShader
    {
        ...
        // Variable para el tiempo real (ignora Time.timeScale)
        float _UnscaledTime;
        ENDHLSL
        ...
        Pass
        {
            ...
            float4 frag(v2f i, float facing : VFACE) : SV_TARGET
            {
                ...
                // CORRECCIÓN: Scanlines usan _UnscaledTime
                screenUVs.y += _UnscaledTime * _ScrollSpeed;
                float3 scanlines = SAMPLE_TEXTURE2D(_ScanlineTex, sampler_LinearRepeat, screenUVs).rgb;

#ifdef _TRACKING_ON
                // CORRECCIÓN: Tracking usa _UnscaledTime
                float2 trackingUVs = float2(UVs.y * _TrackingSize + _UnscaledTime * _TrackingSpeed + randomValue(_UnscaledTime.xx) * _TrackingJitter, 0.5f);
                ...
                // CORRECCIÓN: Random Wear usa _UnscaledTime
                float randomOffset = randomValue(float2(i.uv.y, _UnscaledTime));
...
#ifdef _TRACKING_ON
                // CORRECCIÓN: Tracking Lines usan _UnscaledTime
                float t = _UnscaledTime % 1.0f + 2.307f;
        ...
        // Los demás pases (DepthOnly, etc) no necesitan unscaled time generalmente
    }
}