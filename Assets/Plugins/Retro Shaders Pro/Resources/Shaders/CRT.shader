	...
	SubShader
	{
		...
		Pass
		{
            ...
            float4 frag (Varyings i) : SV_Target
            {
...
#ifdef _TRACKING_ON
				float2 trackingUVs = float2(UVs.y * _TrackingSize + _UnscaledTime * _TrackingSpeed + randomValue(float2(_UnscaledTime, _UnscaledTime)) * _TrackingJitter, 0.5f);
				...
				float randomOffset = randomValue(float2(i.texcoord.y, _UnscaledTime));
